...
export function lineInPolygon(p0: mypoint, p1: mypoint, pl: mypoint[]){
   ...
   if (pointInPolygon(p0,pl) && pointInPolygon(p1,pl)){
    ...
    for(let pl1 of pl){
        for(let pl2 of pl){
            if(lineAndLine(p0, p1, pl1, pl2)){
                lineIntersectsPolygon = 1
                break;
                
            }
            if(lineIntersectsPolygon == 1)
            break;
        }
    } 
